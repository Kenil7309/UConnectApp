...
String URL = "http://192.168.43.215:5000/";

///1 LogIN
class LogIN {
  LogIN();
  Future<LoginModel> Log_In(EmployeeID, password) async {
    var url ="http://192.168.43.215:5000/api/auth/login";
...
class PostingEmptyAttendance {
  ...
  Future<AttendanceModel> CheckIn(
    ...
    var data = json.decode(response.body);
    print(
        "Employee Today's Empty Attendance Added Successfully${response.body}");
...
///6 Getting Specifc Employee
class GettingEmployee {
  GettingEmployee();
  Future<SpecificUserModel> GettingEmployeeData() async {
    final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
    final SharedPreferences prefs = await _prefs;
    var url = URL + "api/users/" + prefs.getString("EmployeeUniqID").toString();
    http.Response response = await http.get(Uri.parse(url), headers: {
      "content-type": "application/json",
      'Cookie': "access_token=${prefs.getString("Token")}"
    });
    var data = json.decode(response.body);
    print("Employee ${response.body}");
    return SpecificUserModel.fromJson(data);
  }
}
...
class UpdatingEmployee {
  UpdatingEmployee();
  Future<SpecificUserModel> UpdatingEmployeeData(name, mobile, email) async {
    final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
    final SharedPreferences prefs = await _prefs;
    var url = URL + "api/users/" + prefs.getString("EmployeeUniqID").toString();
    http.Response response = await http.put(Uri.parse(url),
    ...
    var data = json.decode(response.body);
    print("Update Employee ${response.body}");
    return SpecificUserModel.fromJson(data);
  }
}
